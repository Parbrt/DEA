...
def application_train_test(path_to_data="../dataset/raw/"):
    ...
    console.print(
        "[yellow] Traitement de l'anomalie DAYS_EMPLOYED (365243 -> NaN)...[/yellow]"
    )
    df["DAYS_EMPLOYED"] = df["DAYS_EMPLOYED"].replace(365243, np.nan)

    # --- Feature Engineering ---
    console.print(
        "[blue]Feature Engineering : Création des ratios financiers...[/blue]"
    )
    ...
    console.print(
        f"[green]✔ Traitement initial terminé. Colonnes actuelles : {df.shape[1]}[/green]"
    )
    return df


# Fonctions d'Agrégation (Tables liées)
def bureau_and_balance(df, path_to_data="../dataset/raw/"):
    ...
    with Progress(
        ...
        progress.add_task(
            description="Chargement et agrégation de bureau.csv...", total=None
        )
    ...
    console.print(
        f"[green]✔ Données Bureau fusionnées. Colonnes : {df.shape[1]}[/green]"
    )
    return df
...
def previous_applications(df, path_to_data="../dataset/raw/"):
    ...
    with Progress(
        ...
        progress.add_task(
            description="Chargement et agrégation de previous_application.csv...",
            total=None,
        )
        ...
        prev_agg = prev.groupby("SK_ID_CURR").agg(
            {k: agg_ops[k] for k in existing_cols}
        )
    ...
    console.print(
        f"[green]✔ Données Previous App fusionnées. Colonnes : {df.shape[1]}[/green]"
    )
    return df
...
def main():
    console.print(
        "[bold red]Démarrage du Pipeline de Préparation de Données[/bold red]",
        justify="center",
    )
    ...
    console.print(
        "[bold yellow]Calcul de l'imputation (Médiane) sur le TRAIN...[/bold yellow]"
    )
    ...
    with Progress(
        ...
        train_processed = pd.DataFrame(
            imputer.transform(train_df), columns=train_df.columns
        )
    ...
    print_shape_table(
        "Dimensions Finales des Datasets", train_processed, test_processed
    )
    ...
    console.print(
        "[bold white on green] Pipeline terminé avec succès ! [/bold white on green]"
    )
